...
`data` | [Array&lt;SingleHistoricalBarLast&gt;](SingleHistoricalBarLast.md)
`direction` | number
`high` | string
`low` | string
...
```typescript
import type { IserverHistoryLastResponse } from 'bezant-client'
...
const example = {
  ...
  "travelTime": null,
  "volumeFactor": null,
} satisfies IserverHistoryLastResponse
...
const exampleParsed = JSON.parse(exampleJSON) as IserverHistoryLastResponse
